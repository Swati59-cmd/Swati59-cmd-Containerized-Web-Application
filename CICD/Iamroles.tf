resource "aws_iam_role" "codepipeline_role1" {
  name = "${var.env}-codepipeline-roledemo"
...
resource "aws_iam_role" "codebuild_role" {
  name = "${var.env}-codebuild-service-roledemmo"
...
resource "aws_iam_policy" "codebuild_s3_access" {
  name = "${var.env}-CodeBuildS3Accessdemo"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_policy" "codepipeline_codestar_connection" {
  name = "${var.env}-AllowCodeStarConnectionUsemain"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Effect   = "Allow",
...
resource "aws_iam_policy" "codepipeline_codebuild_access" {
  name = "${var.env}-CodePipelineStartBuildAccessmain"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Effect = "Allow",
...
resource "aws_iam_policy" "codebuild_cloudwatch_logs" {
  name = "${var.env}-CodeBuildCloudWatchLogsAccessmain"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_policy" "codebuild_s3_read" {
  name = "${var.env}-CodeBuildS3ReadAccessmain"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_policy" "codepipeline_ecs_access" {
  name = "${var.env}-CodePipelineECSAccessmain"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
